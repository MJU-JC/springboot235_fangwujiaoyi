        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '房屋编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.zufangUuidNumber" placeholder="房屋编号" clearable></el-input>
                     </el-form-item>
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '开始房屋面积' : ''">
                         <el-input v-model="searchForm.zufangMianjiStartSearch" οnkeyup="value=value.replace(/[^\d]+\./g,'');this.value=/^\d+\.?\d{0,1}$/.test(this.value) ?
                                                                                                  this.value : this.value.split('.')[1].length==1?this.value:this.value=this.value.split('.')[0]+'.'+this.value.split('.')[1].substr(0,1);"
                                   placeholder="请输入开始房屋面积" clearable></el-input>
                     </el-form-item>
                    <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                        =》
                    </el-form-item>
                     <el-form-item :label="contents.inputTitle == 1 ? '结束房屋面积' : ''">
                         <el-input v-model="searchForm.zufangMianjiEndSearch" οnkeyup="value=value.replace(/[^\d]+\./g,'');this.value=/^\d+\.?\d{0,1}$/.test(this.value) ?
                                                                                                  this.value : this.value.split('.')[1].length==1?this.value:this.value=this.value.split('.')[0]+'.'+this.value.split('.')[1].substr(0,1);"
                                   placeholder="请输入结束房屋面积" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '开始现加/月' : ''">
                         <el-input v-model="searchForm.zufangNewMoneyStartSearch" οnkeyup="value=value.replace(/[^\d]+\./g,'');this.value=/^\d+\.?\d{0,1}$/.test(this.value) ?
                                                                                                  this.value : this.value.split('.')[1].length==1?this.value:this.value=this.value.split('.')[0]+'.'+this.value.split('.')[1].substr(0,1);"
                                   placeholder="请输入开始现加/月" clearable></el-input>
                     </el-form-item>
                    <el-form-item :label="contents.inputTitle == 1 ? '' : ''">
                        =》
                    </el-form-item>
                     <el-form-item :label="contents.inputTitle == 1 ? '结束现加/月' : ''">
                         <el-input v-model="searchForm.zufangNewMoneyEndSearch" οnkeyup="value=value.replace(/[^\d]+\./g,'');this.value=/^\d+\.?\d{0,1}$/.test(this.value) ?
                                                                                                  this.value : this.value.split('.')[1].length==1?this.value:this.value=this.value.split('.')[0]+'.'+this.value.split('.')[1].substr(0,1);"
                                   placeholder="请输入结束现加/月" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '房屋状态' : ''">
                        <el-select v-model="searchForm.zufangZhuangtaiTypes" placeholder="请选择房屋状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in zufangZhuangtaiTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zufang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/fangwujiaoyipingtai/upload/zufangMuBan.xls"
                        >批量导入租房数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zufang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zufangUploadSuccess"
                                :on-error="zufangUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zufang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zufang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zufangUuidNumber"
                                   header-align="center"
                                   label="房屋编号">
                        <template slot-scope="scope">
                            {{scope.row.zufangUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="房屋热度">
                        <template slot-scope="scope">
                            {{scope.row.zufangClicknum}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="zufangZhuangtaiTypes"
                                      header-align="center"
                                      label="房屋状态">
                        <template slot-scope="scope">
                            {{scope.row.zufangZhuangtaiValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shangxiaTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('zufang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zufang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('zufang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('zufang','修改')" type="primary" icon="el-icon-tickets" size="mini" @click="shangxia(scope.row.id,scope.row.shangxiaTypes)">{{scope.row.shangxiaTypes == 1?'下架':'上架'}}</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zufang',//当前表表名,
                        sumColum : 'zufang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zufang_name',//分组字符串字段
                        // types : 'zufang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['zufangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            zufangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zufang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
