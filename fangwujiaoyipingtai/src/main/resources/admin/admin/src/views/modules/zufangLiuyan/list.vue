        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zufangLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/fangwujiaoyipingtai/upload/zufangLiuyanMuBan.xls"
                        >批量导入租房留言数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zufangLiuyan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zufangLiuyanUploadSuccess"
                                :on-error="zufangLiuyanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('zufangLiuyan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入租房留言数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zufangLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zufangLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="zufangLiuyanText"
                                      header-align="center"
                                      label="留言内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.zufangLiuyanText != null &&scope.row.zufangLiuyanText.length>10">
                                {{scope.row.zufangLiuyanText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.zufangLiuyanText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('zufangLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zufangLiuyan','修改') && sessionTable=='yonghu' && userId==scope.row.zufangYonghuId" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>

                            <el-button v-if="isAuth('zufangLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zufang_liuyan',//当前表表名,
                        sumColum : 'zufang_liuyan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zufang_liuyan_name',//分组字符串字段
                        // types : 'zufang_liuyan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "zufang_liuyan_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['zufangLiuyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "zufangLiuyan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            zufangLiuyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zufangLiuyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
